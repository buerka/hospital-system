                ...
                <div style={{ maxWidth: '1200px', margin: '-40px auto 0', padding: '0 20px 60px' }}>
                    ...
                    <Row gutter={[32, 32]}>
                        <Col xs={24} sm={12} lg={6}>
                            <Card title="挂号员" bordered={false} className="role-card" extra={<Button type="link" onClick={() => navigate('/dashboard/bookings')}>进入</Button>}>
                                管理窗口挂号与预约分诊。
                            </Card>
                        </Col>
                        <Col xs={24} sm={12} lg={6}>
                            <Card title="临床医生" bordered={false} className="role-card" extra={<Button type="link" onClick={() => navigate('/dashboard/doctor')}>进入</Button>}>
                                电子病历、诊断开方与检查。
                            </Card>
                        </Col>
                        <Col xs={24} sm={12} lg={6}>
                            <Card title="财务收费" bordered={false} className="role-card" extra={<Button type="link" onClick={() => navigate('/dashboard/payment')}>进入</Button>}>
                                订单结算与收银管理。
                            </Card>
                        </Col>
                        <Col xs={24} sm={12} lg={6}>
                            <Card title="物资管理" bordered={false} className="role-card" extra={<Button type="link" onClick={() => navigate('/dashboard/storehouse')}>进入</Button>}>
                                药品库存进销存管理。
                            </Card>
                        </Col>
                    </Row>
